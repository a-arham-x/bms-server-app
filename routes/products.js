...
router.post(
  ...
  [
    body("name", { error: "Name of the Product is Required" }).isLength({
      min: 5,
    }),
    body("price", { error: "Price of the Product is Required" }).isNumeric({
      min: 1,
    }),
  ],
  async (req, res) => {
    ...
    if (key != customer._id)
      return res.json({ message: "Not authorized for making the change" });
...
router.get("/details/:id", async (req, res) => {
  ...
  return res.json({
    "Not Found": "The Product you searched for is currently unavailable",
  });
});
...
router.put("/update/:id", fetchAdmin, async (req, res) => {
  ...
  if (key != customer._id)
    return res.json({
      message: "Not authorized for making the change",
...
router.delete("/delete/:id", fetchAdmin, async (req, res) => {
  ...
  if (ordersProducts.length > 0) {
    for (let i = 0; i < ordersProducts.length; i++) {
      ...
      if (!order.received && !order.cancelled) {
        return res.json({
          message: "Cannot delete the product as there are orders of it made.",
          success: false,
        });
      }
    }
  ...
  await Product.findByIdAndDelete(product._id)
    .then(() => {
      return res.json({ message: "Product Deleted", success: true });
    })
    .catch(() => {
